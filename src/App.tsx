...
import {
  ...
  ListTodo, LogOut, Plus, Search, Settings, Sparkles, Tags, Trash2, X, Zap
} from 'lucide-react';
...
const taskColors: { value: TaskColor; label: string; hex: string }[] = [
  { value: 'violet', label: 'Electric Violet', hex: '#b42cff' }, { value: 'blue', label: 'Sky Blue', hex: '#42a5f5' },
  { value: 'green', label: 'Neon Green', hex: '#54df70' }, { value: 'orange', label: 'Sunset Orange', hex: '#ff983d' },
...
function AuthScreen({ mode, onModeChange, onAuthenticate }: { mode: 'login' | 'signup'; onModeChange: (mode: 'login' | 'signup') => void; onAuthenticate: (username: string, password: string, mode: 'login' | 'signup') => string }) {
  ...
  const submit = (event: FormEvent) => {
    ...
    if (signup && password !== confirm) { 
      setError('Passwords do not match.'); triggerVibrate([50, 50]); return; 
    }
    ...
    <main className="auth-shell">
      <section key={animKey} className="auth-card auth-switch-anim">
        ...
        <div className="auth-copy">
          ...
          <p>{signup ? 'Create your space and make every day count.' : 'Pick up where you left off.'}</p>
        </div>
...
function Dashboard({ username, onLogout }: { username: string; onLogout: () => void }) {
  ...
  const handleClearCompleted = () => { triggerVibrate(50); setTasks((current) => current.filter((t) => !t.completed)); setToast('Completed tasks cleared'); setShowSettings(false); setProfileMenuOpen(false); };
  const handleWipeData = () => { if (confirm('Are you sure you want to delete all tasks?')) { triggerVibrate([100, 100]); setTasks([]); setToast('All tasks deleted'); setShowSettings(false); setProfileMenuOpen(false); } };
  ...
  return <div className="app-shell">
    ...
    <aside className="sidebar">
      ...
      <div className="sidebar-bottom">
        ...
        <div className="profile"><div className="avatar">{username.slice(0, 1).toUpperCase()}</div><div><strong>{username}</strong><small>Personal workspace</small></div><button className="logout-btn" onClick={onLogout}><LogOut size={16} /></button></div>
      </div>
    </aside>
    
    <section className="content">
      {/* UNIVERSAL HEADER (Changes automatically based on screen size) */}
      <header className="topbar">
        <div className="breadcrumb">{activeNav === 'Dashboard' ? 'Your Workspace' : activeNav}</div>
        <div className="mobile-brand">
          <span className="brand-mark"><Zap size={16} fill="currentColor"/></span> Todo
        </div>
        <div className="topbar-actions">
          <button className="notification-btn" onClick={() => { triggerVibrate(20); setToast('No new notifications'); }}>
            <Bell size={20} /><i />
          </button>
          <button className="mobile-profile-btn" onClick={() => { triggerVibrate(20); setProfileMenuOpen(true); }}>
          ...
          <div className="section-heading">
            ...
            <div className="task-tools">
              <div style={{ position: 'relative' }}>
                ...
                {pdfMenuOpen && (
                  <div style={{ position: 'absolute', top: '100%', right: 0, marginTop: '8px', background: '#272532', border: '1px solid rgba(255,255,255,0.1)', padding: '5px', borderRadius: '12px', zIndex: 10, display: 'flex', flexDirection: 'column', gap: '2px', minWidth: '130px', boxShadow: '0 10px 25px rgba(0,0,0,0.5)' }}>
                    {['Daily', 'Weekly', 'Monthly', 'Yearly'].map(tf => (
                      <button key={tf} type="button" onClick={() => { downloadReport(tf as any); setPdfMenuOpen(false); }} style={{ background: 'transparent', border: 'none', color: '#fff', padding: '10px 12px', textAlign: 'left', fontSize: '13px', borderRadius: '8px' }} onMouseEnter={(e) => e.currentTarget.style.background = 'rgba(255,255,255,0.08)'} onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'}>
                        {tf} Tasks
                      </button>
                    ))}
                  </div>
    ...
    {/* MOBILE ONLY PROFILE MENU SHEET */}
    {profileMenuOpen && (
      <div className="modal-backdrop" onClick={() => setProfileMenuOpen(false)}>
        <div className="task-modal" onClick={e => e.stopPropagation()} style={{ padding: '30px 20px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '15px', marginBottom: '25px', borderBottom: '1px solid rgba(255,255,255,0.08)', paddingBottom: '20px' }}>
            ...
            <button onClick={() => setProfileMenuOpen(false)} style={{ marginLeft: 'auto', background: 'rgba(255,255,255,0.08)', border: 0, color: '#fff', padding: '8px', borderRadius: '50%' }}><X size={18}/></button>
          </div>
    ...
    {toast && <div className="toast"><Bell size={18} /><span>{toast}</span></div>}
  </div>;
}
    ...
    <article className={`task-card ${task.color} ${task.completed ? 'done' : ''}`} style={isOverdue ? { borderColor: 'rgba(255, 80, 80, 0.5)', backgroundColor: 'rgba(255, 80, 80, 0.05)' } : {}}>
      ...
      <div className="task-info">
        ...
        <div className="task-meta">
          ...
          <span className="category-pill" style={{border: '1px solid currentColor', opacity: 0.8}}>⚡ {task.priority}</span>
        </div>
      </div>
